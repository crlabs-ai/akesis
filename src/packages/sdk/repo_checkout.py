    ...
    def checkout_commit(
        ...
        try:
            ...
            raise RepositoryCheckoutError(
                f"Failed to checkout commit {commit_sha} in {repo_owner}/{repo_name}: {err}"
            ) from err
        except Exception as err:
            logger.error("repository_error", error=str(err))
            raise RepositoryCheckoutError(f"Unexpected repository checkout error: {err}") from err
